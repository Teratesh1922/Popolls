...
typedef struct {
    int id;
    const char *url;
    int assigned_per_burst; /* number of requests this thread should send each burst */
...
static void *worker_thread(void *arg) {
    ...
    while (!*(w->shutdown)) {
        ...
        /* perform assigned requests for this burst */
...
int main(int argc, char **argv) {
    const char *url = NULL;
    int r = -1; /* requests per interval */
    int t = 1;  /* threads */
    int m = 1000; /* ms interval */
    int s = 10; /* total seconds */
    ...
    /* Safety: user said target is their own — still print a warning */
    ...
    /* compute per-thread assignment for each burst */
